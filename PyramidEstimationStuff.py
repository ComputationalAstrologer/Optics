...
parms['lens1_fill_diameter'] = parms['beam_diameter']*5  #  computational beam width at lens#1.  This matters!
...
parms['max_chirp_step_deg'] = 10  # maximum allowed value (degrees) in chirp step for Fresnel prop
parms['max_lens_step_deg'] = 10 # maximum step size allowed for lens phase screen
parms['max_plane_step_deg'] = None  # (degrees) maximum phase step allowed for planar phase screen
parms['max_pyramid_phase_step'] = 10  # maximum step (degrees) allowed for pyramid phase ramp
...
ReduceF4sysmat = False
if ReduceF4sysmat:
    nnx = 138
    sm = np.zeros((nnx*nnx, nppix)).astype('complex')
    for k in range(nppix):
        v = sysmat[:,k].reshape(712,712)
        vr = dc(v, 4, ftype='fir', axis=0, zero_phase=True)
        vr = dc(vr, 4, ftype='fir',axis=1,zero_phase=True)
        vr = vr[20:158,20:158]
        sm[:,k] = vr.reshape(nnx*nnx,)

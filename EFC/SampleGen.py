...
def GenerateFieldPairs(Npairs, setup=defaultsetup):
   ...
   imshape = ( setup['corners'][3] - setup['corners'][2], setup['corners'][1] - setup['corners'][0] )
   A = EFC.EFC(HolePixels=pl, SpeckleFactor=0.)
   Czero = np.zeros((A.Shx.shape[1],))
   f0x = A.Field(Czero,'X','Hole','phase',False,0.).reshape(imshape)  # get nominal field the region
   f0y = A.Field(Czero,'Y','Hole','phase',False,0.).reshape(imshape)
   ...
   for ks in range(Npairs):
       ...
       f1x = A.Shx@phasor_coef - f0x
       f1y = A.Shy@phasor_coef - f0y
